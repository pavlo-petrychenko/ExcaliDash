...
(pattern) => pattern.test(value) ); const isTooLarge = value.length > MAX_DATAURL_SIZE; if (hasSuspiciousContent || isTooLarge) { file[key] = ""; } else { file[key] = value; } } else if (/^https?:\/\ const hasSuspiciousContent = suspiciousPatterns.some(
